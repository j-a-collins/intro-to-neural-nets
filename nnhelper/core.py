"""Neural Net Helper: Core class
"""

import numpy as np


# Initialises nnhelper
def init(dot_precision_workaround=True, default_dtype="float32", random_seed=0):
    ...
    # make computations using float64, but return float32 data
    if dot_precision_workaround:
        orig_dot = np.dot

        def dot(*args, **kwargs):
            return orig_dot(*[a.astype("float64") for a in args], **kwargs).astype(
                "float32"
            )
